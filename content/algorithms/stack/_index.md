...
<details>
<summary>
A string $s$ that is composed of parentheses represents an expression, where the expression is
computed to a single value $f(s)$. Given a string $s$, compute $f(s)$.
</summary>

### Related Problems

- Baekjoon 10799: $f(x)$ is the number of blocks generated by cutting the string in adjacent parentheses.
- Baekjoon 2504: $f(s)$ is computed by the following rules:
  $\text{()}\in S, s\in S\rarr f(\text{(s)}) = 2f(s)$.
